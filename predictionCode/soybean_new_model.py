...
var_base = {'T_linear_6_8': " + tave6 + tave7 + tave8",
            ...
            'LSTMAX_spline_6_8': (
                        "+ bs(lstmax6, df=4, knots = (28.6, 33.1, 34.5), degree=1,lower_bound=20,upper_bound=50)"
...
def init_model(df_train, df_test, model_txt, yield_type='rainfed', weight=False):
    ...
    return results, df_test.copy().join(
        results.predict(df_test).to_frame('Predicted_' + yield_type_dict[yield_type] + '_ana'))
...
def get_prediction_for_year(df, model_type, y, yield_type, prediction_type='forward',
    ...
    if (train_data['yield_irr'].isnull().all()) & (yield_type == 'irrigated'):
        ...
        df_predict['Predicted_' + yield_type_dict[yield_type]] = (
                    df_predict['Predicted_' + yield_type_dict[yield_type] + '_ana']
...
def get_prediction_year_range(df, model_type, prediction_type='forward', yield_type='rainfed',
    ...
    if yield_type != 'all':
        frame = [get_prediction_for_year(df, model_type, i, yield_type, prediction_type=prediction_type,
                                         state_subset=state_subset, fix=fix) for i in
                 range(test_start_year, year_range[1] + 1)]
        ...
        df_out = pd.concat(frame1).merge(
            pd.concat(frame2).loc[:, ('year', 'FIPS', 'Predicted_yield_irr_ana', 'Predicted_yield_irr')])

        df_out['Predicted_yield'] = (df_out.fillna(0)['Predicted_yield_rainfed'] * df_out.fillna(0)['area_rainfed']
                                     + df_out.fillna(0)['Predicted_yield_irr'] * df_out.fillna(0)['area_irr']) / (
                                                df_out.fillna(0)['area_rainfed']
...
def load_yield_data():
    ...
    data_12.loc[:, 'yield_rainfed_ana'] = (
                data_12['yield_rainfed'] - trend_rainfed.predict(data_12[['year', 'yield_rainfed']]))
...
def save_prediction(df, yield_type='rainfed', prediction_type='forward'):
    ...
    svd_issue = ['evi_linear_only', 'lstmax_spline_evi_poly_only', 'vpd_spline', 'vpd_spline_evi',
                 'vpd_spline_evi_poly', 'lstmax_spline_evi_poly',
                 'lstmax_spline_evi_poly_only']  # all, leave_one_year_out
...
def prediction_result_global(test, yield_type='rainfed'):
    ...
    for y in range(test['year'].min(), test['year'].max() + 1):
        ...
        sse = ((test.loc[con, yield_type_dict[yield_type]] - test.loc[
            con, 'Predicted_' + yield_type_dict[yield_type]]) ** 2).sum()
...
def prediction_result_by_state(test, yield_type='rainfed'):
    ...
    for y in range(test['year'].min(), test['year'].max() + 1):
        for st in test['State'].unique():
            ...
            # N is the sample number after removing nan
            N = test.loc[con, [yield_type_dict[yield_type], 'Predicted_' + yield_type_dict[yield_type]]].dropna().shape[
                0]
