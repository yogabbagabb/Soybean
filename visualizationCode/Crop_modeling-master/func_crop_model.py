...
# Calculate global prediction performance, by lumping all predictions years together 
def prediction_result_global_lumped(test,yield_type='rainfed'):

    r2_temp = test.loc[:,[yield_type_dict[yield_type],
                            'Predicted_'+yield_type_dict[yield_type]]].corr() \
        ['Predicted_'+yield_type_dict[yield_type]][0]**2
    
    # N is the sample number after removing nan
    N = test.loc[:,[yield_type_dict[yield_type],'Predicted_'+yield_type_dict[yield_type]]].dropna().shape[0]
    rmse_temp = (((test.loc[:, 'Predicted_'+yield_type_dict[yield_type]] - 
                      test.loc[:, yield_type_dict[yield_type]])**2).sum() \
                                  /N)**0.5

    sst = ((test.loc[:, yield_type_dict[yield_type]] 
            - test.loc[:, yield_type_dict[yield_type]].mean())**2).sum()
    sse = ((test.loc[:, yield_type_dict[yield_type]] - test.loc[:, 'Predicted_'+yield_type_dict[yield_type]])**2).sum()

    print('R2 is %f, RMSE is %f, and classic R2 is %f'%(r2_temp, rmse_temp, 1-sse/sst))
